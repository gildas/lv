...
// GetProviders gets the pods for the current context
func GetProviders(ctx context.Context, cmd *cobra.Command, args []string, toComplete string) ([]string, error) {
	...
	err = New().Exec(ctx, []string{"get", "deployments.apps", "--context", kubectlContext, "--namespace", kubectlNamespace, "-o", "jsonpath={.items[*].metadata.labels.provider}"}, &stdout, &stderr)
	if err != nil {
	...
	return common.FilterValidArgs(providers, args, toComplete), nil
}
